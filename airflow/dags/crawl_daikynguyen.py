...
def crawl_daikynguyen_health(
    ...
    **kwargs,
    ...
    """Crawl health news from Dai Ky Nguyen ."""
    ...
    try:
        for i in range(start_page, end_page + 1):
            logger.info(f"\nLoading page {i}...")
            try:
                driver.get(f"https://www.dkn.tv/cat/doi-song/suc-khoe/page/{i}")
            ...
            for art in articles:
                title_tag = art.find("div", class_="p-entry")
                link_tag = title_tag.find("a") if title_tag else None
                summary_tag = title_tag.find("p", class_="p-excerpt") if title_tag else None
                ...
                meta_data = title_tag.find("p", class_="p-meta") if title_tag else None
                time_tag = (
                    meta_data.find("span", class_="tie-date").find("span", class_="p-time")
                    if meta_data
                    else None
                )
                published_date = None
                if time_tag:
                    text = time_tag.get("data-date")
...
# ---- Define the DAG ----
with DAG(
    dag_id="daikynguyen_health_crawl_dag",
    default_args=default_args,
    description="Crawl Dai Ky Nguyen Health News and save to CSV",
    schedule="0 6 * * *",  # every day at 6:00 AM
    start_date=datetime(2025, 10, 20),
    catchup=False,
    tags=["crawl", "selenium", "Daikynguyen"],
) as dag:

    def crawl_task(**context):
        output_csv = "/opt/data/daikynguyen_news.csv"
        browser = "chrome"
        driver_path = "chromedriver-linux64/chromedriver"

        data_df = crawl_daikynguyen_health(
    ...
    def insert_and_update_task(**context):
        ti = context["ti"]
        data_df = ti.xcom_pull(task_ids="crawl_daikynguyen_health", key="data")
        insert_or_update(data_df, engine=engine, mode="upsert")

    # ---- PythonOperator ----
    crawl_daikynguyen = PythonOperator(
        task_id="crawl_daikynguyen_health",
        python_callable=crawl_task,
    )

    insert_and_update = PythonOperator(
        task_id="insert_and_update_daikynguyen_data",
        python_callable=insert_and_update_task,
    )

    crawl_daikynguyen >> insert_and_update
